    ...
    function show(heading, description = "", download = null) {
      ...
      if (download) {
        ...
        if (total > 0) {
          ...
          transfer.textContent = `${mib(loaded)} / ${mib(total)} \xB7 ${Math.min(100, Math.floor(loaded / total * 100))}%`;
    ...
    const loading = globalThis.dlssLoading = {
      ...
      runtime(message, error = false, download = null) {
        ...
        if (download) {
          ...
          show("Preparing DLSS for your GPU", compiling ? `Preparing GPU programs${count ? ` \xB7 ${count}` : ""}` : /requesting|adapter/i.test(message) ? "Connecting to your GPU" : "Loading neural models and preparing frame buffers");
        }
      },
      finish() {
